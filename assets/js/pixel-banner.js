...
function configureOptions(src, opts, target_ref) {
  ...
  if (src.options.dynamic_options.dynamic) {
    window.setInterval(function () {
      dynamicIntervalUpdate(src);
    }, src.options.dynamic_options.update_interval);
  }
  ...
  if (src.options.pixel_options.probability.x_axis == "distance")
    src.options.pixel_options.probability.x_axis =
      probabilityFromDistanceToCenter;
  if (src.options.pixel_options.probability.y_axis == "distance")
    src.options.pixel_options.probability.y_axis =
      probabilityFromDistanceToCenter;
}
...
function dynamicIntervalUpdate(src) {
  if (src.options.dynamic_options.update_mode == "all")
    ...
    drawRandomPixel(
      src.canvas,
      src.options.pixel_options,
      src.options.dynamic_options
    );
}
...
function drawPixel(x, y, w, h, color, ctx, opt, alpha) {
  ...
  ctx.fillStyle =
    color ||
    opt.color_pallete[Math.floor(Math.random() * opt.color_pallete.length)];
...
function drawRandomPixel(c, opt, anim_opt) {
  ...
  if (calculateProbability(opt.probability, x, y, c)) {
    ...
    replacePixel(
      ...
      opt,
      0,
      anim_opt.fade_interval_duration
    );
  }
  //else
...
function probabilityAlways(position, length) {
  return 1;
}
